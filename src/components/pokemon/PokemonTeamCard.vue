<template>
    <BaseCard class="hover:shadow-xl transition-all duration-300 my-2">
        ...
        <div class="flex flex-col md:flex-row items-center gap-2 px-1 border-b border-gray-200 mb-4">
            ...
            <h3 class="text-lg font-bold text-gray-900 dark:text-gray-50 capitalize cursor-pointer hover:text-blue-600 transition-colors"
                @click="$emit('view-detail', pokemon.id)">
                {{ pokemon.name }}
            </h3>
            <span class="text-xs text-gray-500 dark:text-gray-200">#{{ pokemon.id.toString().padStart(3, '0') }}</span>
        ...
        <div class="flex flex-col rounded-lg gap-4">
            <div class="grid grid-cols-1 md:grid-cols-2 items-start gap-4 w-full">
                ...
                <div class="relative w-full cursor-pointer" @click="$emit('view-detail', pokemon.id)">
                    <div class="w-full aspect-square bg-linear-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 rounded-lg p-4 flex items-center justify-center relative overflow-hidden">
                ...
                <!-- Información (Grafico) -->
                <div class="relative w-full h-full border border-gray-200 dark:border-gray-700 rounded-lg">
                    <div class="font-medium py-2 text-center text-gray-900 dark:text-gray-50 border-b border-gray-200 dark:border-gray-700">Estadísticas</div>
